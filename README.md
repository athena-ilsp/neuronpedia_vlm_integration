...
This fork extends Neuronpedia so the existing UI (activation search, feature pages, steering, token heatmaps, circuit tracing) works with **Vision-Language Models** — specifically `google/gemma-3-4b-it` and custom SAEs/transcoders trained on its activations. The original codebase targets text-only language models with [sae-lens](https://github.com/jbloomAus/SAELens) SAEs loaded from HuggingFace; that path is preserved, and a parallel VLM path was added that routes images and text through the same UI, the same activation endpoints, and the same circuit-tracing pipeline. For the full technical writeup see [VLM_INTEGRATION_REPORT.md](VLM_INTEGRATION_REPORT.md).

## Inference server — [apps/inference/](apps/inference/)

Neuronpedia assumes a `HookedTransformer` (text-only, hooks already exposed); Gemma-3 VLM is a HuggingFace `Gemma3ForConditionalGeneration` with image preprocessing and no built-in activation hooks. The inference server was extended to host this model class alongside the existing text-only path.

- **Model adapter** ([models/vlm_model_adapter.py](apps/inference/neuronpedia_inference/models/vlm_model_adapter.py)) — a wrapper that exposes the HuggingFace VLM through the `HookedTransformer` interface. It owns the processor (chat-template tokenization and image patch encoding), retains the current image for use in the next forward pass, and registers MLP-in/MLP-out hooks on each transformer layer so SAEs can be attached at the same points used during training.
- **Custom SAE loader** ([saes/vlm_sae.py](apps/inference/neuronpedia_inference/saes/vlm_sae.py)) — sae-lens cannot load our `.pt` checkpoints, so this wrapper loads the trained `SparseAutoencoder` and exposes it through the interface the rest of Neuronpedia expects.
- **SAE manager** ([sae_manager.py](apps/inference/neuronpedia_inference/sae_manager.py)) — extended to read a `VLM_SAE_PATHS` JSON env var, attach each SAE to its corresponding hook, and bypass the usual auto-load/unload behavior (VLM SAEs remain pinned in memory).
- **Bootstrapping** ([args.py](apps/inference/neuronpedia_inference/args.py), [config.py](apps/inference/neuronpedia_inference/config.py), [shared.py](apps/inference/neuronpedia_inference/shared.py), [server.py](apps/inference/neuronpedia_inference/server.py)) — a `VLM=true` switch routes startup through the new path: load Gemma-3 and its processor, wrap in the adapter, and skip sae-lens directory lookups.
- **Activation endpoints** ([endpoints/activation/](apps/inference/neuronpedia_inference/endpoints/activation/)) — every endpoint that previously accepted text now also accepts an optional image (`image_base64`). The image is set on the adapter before each forward pass so it is fused into the input. `activation/all.py` additionally zeroes out activations on chat-template control tokens (`<bos>`, `<start_of_turn>`, `<end_of_turn>`, etc.), since the SAE was not trained on these tokens and they otherwise dominate feature rankings; it also supports a top-K-per-token filter for cleaner search results.

## Graph server — [apps/graph/](apps/graph/)

The graph server runs [circuit-tracer](https://github.com/safety-research/circuit-tracer) to build attribution graphs. Its default behavior is to load the model onto a single GPU at full precision, which is not viable for a 4B-parameter VLM with multiple transcoder SAEs loaded alongside it.

- **Runtime patches to ReplacementModel** ([neuronpedia_graph/server.py](apps/graph/neuronpedia_graph/server.py)) — at startup, circuit-tracer's `ReplacementModel` is monkey-patched to honor two new env vars: `GRAPH_DEVICE_MAP` (HuggingFace `accelerate`-style placement, e.g. spreading layers across GPUs) and `GRAPH_MAX_MEMORY` (per-GPU memory caps). This allows a single host to run the VLM and its transcoders for attribution without OOMing.
- No explicit image-aware code was required in the graph server itself — circuit-tracer operates on whichever model object is supplied, and the patched device-map logic is sufficient to load and run the VLM.

## API client packages — [packages/](packages/)

The Python and TypeScript clients are generated from the inference server's OpenAPI schema, so adding image support required extending the wire format end-to-end.

- **Python client** ([packages/python/neuronpedia-inference-client/](packages/python/neuronpedia-inference-client/)) — `image_base64` and `top_k` fields added to every activation request model.
- **TypeScript client** ([packages/typescript/neuronpedia-inference-client/](packages/typescript/neuronpedia-inference-client/)) — the same fields exposed as `imageBase64` and `activationThreshold` in camelCase, serialized to snake_case on the wire.

## Webapp — [apps/webapp/](apps/webapp/)

Frontend changes are additive: existing forms and graph views were adapted to handle images and the new token types Gemma-3 introduces, without forking the UI for VLMs.

- **Database seeding** ([prisma/seed-vlm.ts](apps/webapp/prisma/seed-vlm.ts), [prisma/cleanup-vlm.ts](apps/webapp/prisma/cleanup-vlm.ts)) — registers the Gemma-3 model, the VLM source sets (SAE and transcoder), and one source per layer in the local Postgres so the model and its features appear in the model picker.
- **VLM detection** ([components/inference-searcher/](apps/webapp/components/inference-searcher/), [components/activation-single-form.tsx](apps/webapp/components/activation-single-form.tsx)) — a `VLM_MODELS_PREFIX = ['gemma-3-']` check determines whether to render the image-upload controls. No new pages or modals are introduced; the controls are added inline to existing forms.
- **Image upload and threshold controls** — file-to-base64 conversion is performed client-side, with optional "activation threshold" and "top-K" inputs to manage the dense activation maps that VLMs produce.
- **Token display** — the 256 `<image_soft_token>` entries that Gemma-3 inserts per image would otherwise overwhelm every token chip and graph node. They are collapsed into readable `Image_patch_N` labels in the activation views ([inference-searcher](apps/webapp/components/inference-searcher/)) and into grouped chips in the graph toolbar, info modal, and generate modal ([app/[modelId]/graph/](apps/webapp/app/[modelId]/graph/)).
- **Patch-aware graph rendering** ([app/[modelId]/graph/image-patch-strip.tsx](apps/webapp/app/[modelId]/graph/image-patch-strip.tsx), [link-graph.tsx](apps/webapp/app/[modelId]/graph/link-graph.tsx), [wrapper.tsx](apps/webapp/app/[modelId]/graph/wrapper.tsx)) — when a graph is generated from an image, the page renders the original image together with a strip of the 16×16 patch tiles; graph nodes for each patch position display the corresponding cropped region instead of an `<image_soft_token>` chip.
- **Request plumbing** ([lib/utils/inference.ts](apps/webapp/lib/utils/inference.ts), [lib/utils/graph.ts](apps/webapp/lib/utils/graph.ts), [app/api/search-all/](apps/webapp/app/api/search-all/), [app/api/activation/new/](apps/webapp/app/api/activation/new/)) — propagates `imageBase64` and `topK` from the UI through the webapp's API routes to the inference server. Image-only searches (no text) are now permitted, and the URL cache is bypassed for image queries since the payload does not fit in a query string. `GRAPH_BATCH_SIZE` was reduced (48 → 8) so circuit-tracer fits on a 24 GB GPU when the transcoders are loaded alongside the VLM.
